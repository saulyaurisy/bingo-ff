...
function setupAdminListeners() {
    ...
    adminUnsubscribes.push(unsubTeams);
    ...
    adminUnsubscribes.push(unsubObj);
    ...
    adminUnsubscribes.push(unsubPartida);
}

// ===== VALIDACIÓN CON CARTILLA VISUAL =====
function populateTeamSelect() {
    ...
    if (currentValue && adminAllTeams.some(t => t.id === currentValue)) {
...
function loadTeamObjectives() {
    ...
    if (!teamId) {
        container.innerHTML = '<p style="color:var(--text-secondary);">Selecciona un equipo para ver su cartilla.</p>';
        return;
    }
    ...
    let html = `
        <div style="background:${team.color || '#888'}20; border:1px solid ${team.color || '#888'}; border-radius:10px; padding:12px; margin-bottom:15px; text-align:center;">
    ...
    adminAllObjectives.forEach((obj, index) => {
        ...
        html += `
            <div class="bingo-item ${isCompleted ? 'completed' : ''} ${enLinea && isCompleted ? 'bingo-line' : ''}" 
                 onclick="toggleObjectiveAdmin('${teamId}', '${obj.id}', ${!isCompleted})"
                 style="cursor:pointer;">
                <div class="item-position">${fila + 1},${columna + 1}</div>
                <img src="${obj.imagen}" alt="${obj.nombre}" class="item-img"
                     onerror="this.src='https://via.placeholder.com/40/2a2a3a/ffffff?text=${encodeURIComponent(obj.nombre.charAt(0))}'">
    ...
    if (lineasCompletadas.length > 0) {
        html += `
            <div style="background:rgba(255,215,0,0.1); border:1px solid #ffd700; border-radius:8px; padding:10px; text-align:center;">
                🏆 <strong>Líneas completadas:</strong> 
                ${lineasCompletadas.map(l => `${l.nombre} (+${l.puntos})`).join(', ')}
            </div>
        `;
    }
...
async function toggleObjectiveAdmin(teamId, objectiveId, approve) {
    ...
    if (approve && !completedIds.includes(objectiveId)) {
        ...
        const totalBingoPoints = remainingLines.reduce((sum, l) => sum + l.puntos, 0);
        const basePoints = completedIds.reduce((sum, id) => {
            const obj = adminAllObjectives.find(o => o.id === id);
            return sum + (obj?.puntos || 0);
        }, 0);
        puntos = basePoints + totalBingoPoints;
        bingosGanados = [];
    }
        ...
        switch (action) {
            case 'iniciar':
                ...
                await partidaRef.set({
                    estado: 'en_curso', pausado: false,
                    tiempo_inicio: firebase.firestore.Timestamp.fromMillis(ahora),
                    tiempo_total: 1800, tiempo_restante: 1800
                }, { merge: true });
                db.collection('notificaciones').add({
            ...
            case 'pausar':
                if (currentPartidaData) {
                    const transcurrido = Math.floor((ahora - (currentPartidaData.tiempo_inicio?.toMillis() || ahora)) / 1000);
                    const restante = Math.max(0, (currentPartidaData.tiempo_total || 0) - transcurrido);
            ...
            case 'reanudar':
                if (currentPartidaData?.pausado) {
                    await partidaRef.update({
                        pausado: false,
                        tiempo_inicio: firebase.firestore.Timestamp.fromMillis(ahora),
                        tiempo_total: currentPartidaData.tiempo_restante || 0,
                        estado: 'en_curso'
                    });
                    db.collection('notificaciones').add({
            ...
            case 'reiniciar':
                await limpiarNotificaciones();
                await partidaRef.set({
                    estado: 'esperando', pausado: true,
                    tiempo_inicio: null, tiempo_total: 1800, tiempo_restante: 1800
                });
                const batch = db.batch();
                adminAllTeams.forEach(team => {
...
function addTime(seconds) {
    ...
    const nuevoTotal = (currentPartidaData.tiempo_total || 0) + seconds;
    db.collection('partida').doc('actual').update({
        tiempo_total: nuevoTotal, tiempo_restante: nuevoTotal,
        tiempo_inicio: firebase.firestore.Timestamp.fromMillis(Date.now())
    });
    db.collection('notificaciones').add({
        mensaje: `⏱ +${seconds/60} minutos agregados`,
        timestamp: firebase.firestore.FieldValue.serverTimestamp()
    });
...
function setManualTime() {
    ...
    db.collection('partida').doc('actual').update({
        tiempo_total: seconds, tiempo_restante: seconds,
        tiempo_inicio: firebase.firestore.Timestamp.fromMillis(Date.now()),
        pausado: false, estado: 'en_curso'
    });
...
function updateAdminTimer() {
    ...
    const update = () => {
        if (currentPartidaData.pausado) {
            display.textContent = formatTimeAdmin(currentPartidaData.tiempo_restante || 0) + ' ⏸';
            return;
        }
        const ahora = Date.now();
